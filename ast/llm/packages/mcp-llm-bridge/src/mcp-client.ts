...
export class MCPClient {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.debug(
        `[MCP Client] Server capabilities: ${JSON.stringify(this.serverCapabilities)}`,
      );
    } catch (error: any) {
      logger.error(
  ...
  private handleResponse(data: Buffer) {
    ...
    for (const message of messages) {
      try {
        ...
        logger.error(
          `[MCP Client] Failed to parse response: ${error?.message || String(error)}`,
        );
        console.log({ failedMessage: message });
      }
    }
  }
  ...
  async callTool(toolName: string, toolArgs: any): Promise<any> {
    ...
    logger.debug(
      `[MCP Client] Calling tool '${toolName}' with args: ${JSON.stringify(toolArgs)}`,
    );

    // Check if the tool exists
    if (!this.availableTools.has(toolName)) {
      logger.error(
        `[MCP Client] Unknown tool '${toolName}'. Available tools: ${Array.from(this.availableTools).join(", ")}`,
      );
    }
